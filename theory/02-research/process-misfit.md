...
**Abstract.** Packaged software rarely matches the workflow of the organization installing it. Soh, Kien and Tay-Yap named that gap *misfit*, and showed that a buyer who hits one picks from exactly four responses: change the organization to match the software, accept the shortfall, build a workaround, or customize the package. Strong and Volkoff spent three years inside a single enterprise system implementation and decomposed misfit into six domains, each running in two directions. The consequence for ILG is that implementation cost belongs to neither party on its own. It is a property of the distance between a specific buyer and a specific product, which means the seller can measure it before signature and no feature list will ever reveal it.
...
**Open question this source raises.** The [Deal Triage Calculator](../../practice/01-field-assets/deal-triage-calculator.md) treats workflow maturity as a precondition gate and workflow divergence as a multiplier on the implementation component. The misfit literature suggests workflow codification and implementation cost are coupled rather than independent, because codification is itself a relationship-specific investment. Resolving this requires deciding whether workflow definition and workflow divergence are one variable or two. Constitution v17.0 retired the market stage axis, which removed one of the three the question originally named, and left the remaining pair unresolved.
