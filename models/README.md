...
`.py` files need no frontmatter, and [`check_frontmatter.py`](../practice/02-internal-ops/linting/check_frontmatter.py) scopes itself to `theory/` and `practice/`, so this README is out of its scope. It is still inside the scope of `check_playbook.py` and Vale.
...
**3. The retired panels used market vocabulary that exists nowhere else.** The replaced PNGs labelled their x-axis regions "Nascent, Efficient, Saturated". [process-calculator.md](../practice/01-field-assets/process-calculator.md) step 1 names the stages "Nascent, Transitional, Mature", and the middle two of the panel's labels appear in no other file. The replacement figures do not use market-stage regions at all.
...
- [practice/02-internal-ops/linting/](../practice/02-internal-ops/linting/) for the two checkers this one sits alongside in `.githooks/pre-commit`.
